...
class SendMail
{
    ...
    public function setFrom($email, $name = null)
    {
		...
		$this->MailFrom = ($name) ? "=?UTF-8?B?".base64_encode($name)."?=" . ' <' . $email . '>' : $email;		
        ...
        if($this->SMTPServer) {					
            ...
            if(count($this->MailTo) > 0) {            							// 받는 메일이 있으면 다음 작업을 반복한다
                foreach($this->MailTo as $Email => $Name) {
					...
					$mailTo = ($Name) ? "=?UTF-8?B?".base64_encode($Name)."?=" . ' <' . $Email . '>' : $Email;    // 받는사람					
                    ...
                    if($this->Socket) {
                        ...
                        if(strpos($Result, '250 ok') !== false) $Succ++;        // 성공여부판단
    ...
    public function setContents ($buffer) {
